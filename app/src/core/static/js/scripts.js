...
function openPaymentSimulator() {
  ...
  if (!total || total <= 0) {
    try { showToast('info', 'Agrega productos al carrito para simular pagos.'); } catch (_) {}
    return;
  }
  ...
  if (modalEl && typeof bootstrap !== 'undefined' && bootstrap.Modal) {
    ...
    const deliverAmount = () => {
      if (window.paymentSimulatorInline && typeof window.paymentSimulatorInline.ensureReady === 'function') {
        ...
        try {
          window.setCartAmount(total);
        } catch (err) {
          console.error('Error enviando el monto al simulador:', err);
        }
      } else {
  ...
  const base = (window.SIMULATOR_EXT_URL || window.SIMULATOR_V5_URL || '/maestros/simulador/').trim() || '/maestros/simulador/';
  const url = base + (base.includes('?') ? '&' : '?') + 'total=' + total.toFixed(2) + '&ts=' + Date.now();
  const win = window.open(url, '_blank');
  if (win) {
    const pushAmount = () => {
      try { win.postMessage({ type: 'set_cart_amount', cart_amount: total }, '*'); } catch (_) {}
    };
...
function applySimulatorSelectionToCart(envelope){
  ...
  try {
    ...
    const payments = (Array.isArray(envelope.lines) ? envelope.lines : []).map((line) => ({
      ...
      adquiriente: line.acquirer_label || line.acquirer || '',
    ...
    cart.pagos = payments;
    if (window.state && state.carrito) {
      state.carrito.pagos = payments;
      state.carrito.simulator_totals = envelope.totals || null;
    }
    cart.simulator_totals = envelope.totals || null;
    if (typeof renderSimuladorPagosPOS === 'function') {
      renderSimuladorPagosPOS();
    }
    try { showToast('success', 'Medios de pago actualizados desde el simulador.'); } catch (_) {}
  } catch (err) {
    console.error('applySimulatorSelectionToCart error', err);
    try { showToast('danger', 'No se pudo aplicar el resultado del simulador.'); } catch (_) {}
  }
}
...
window.addEventListener('message', (event) => {
  try {
    ...
    if (data && data.type === 'simulator:payment-selection') {
      applySimulatorSelectionToCart(data);
    }
  } catch (err) {
...
window.addEventListener('simulator:payment-selection', (event) => {
  try {
    const envelope = event?.detail;
    if (envelope && envelope.type === 'simulator:payment-selection') {
      applySimulatorSelectionToCart(envelope);
    }
  } catch (err) {
    console.error('Simulator custom event handler error', err);
  }
});
