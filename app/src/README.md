...
- Containers:
  - `web` (Gunicorn + Django at 8000)
  - `nginx` (serves `/static/` and proxies to web)
- Volúmenes (persistencia):
  - `data` -> `/data` (db.sqlite3 via symlink, sesiones, `services_db` y `cache`)
  - `logs` -> `/app/logs`
  - `staticfiles` -> `/app/staticfiles` (archivos estáticos colectados)
- Variables de entorno útiles (compose):
  - `DJANGO_DEBUG=0`
  - `DJANGO_ALLOWED_HOSTS="tu-dominio.com www.tu-dominio.com"`
  - `SERVICES_DB_DIR=/data/services_db`
  - `SERVICES_CACHE_DIR=/data/cache`
  - `SIMULATOR_V5_EXTERNAL_BASE_URL="http://<host>:8001/maestros/simulador/"`
- Production tip:
  - Change port mapping to `80:80` under `nginx` service.

### Dominio QA (HTTPS)

- Ajusta `docker-compose.yml`:
  - `DJANGO_ALLOWED_HOSTS="mypos-qa.familiabercomat.com"`
  - `DJANGO_CSRF_TRUSTED_ORIGINS="https://mypos-qa.familiabercomat.com"`
  - Cambia puertos de `nginx` a `80:80` (y `443:443` si TLS en Nginx).
- TLS en Nginx (opcional):
  - Usa `nginx-ssl.conf.example` como base y monta tus certificados en `/etc/ssl/familiabercomat/`.
  - Monta el archivo como: `./nginx-ssl.conf.example:/etc/nginx/nginx.conf:ro` y publica `443:443`.
